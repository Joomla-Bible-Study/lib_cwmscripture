(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const doFetch = async (reference, version) => {
            ...
            const response = await window.CwmScriptureFetch.fetch(
        ...
        const fetchPassage = async (switcher, version, label) => {
            ...
            if (!ajaxBaseUrl) {
                if (isDebug) {
                    console.warn('[CwmScripture] Scripture switcher: no AJAX URL configured');
                }
                ...
                try {
                    ...
                    if (!data.retryable || attempt >= MAX_AUTO_RETRIES) {
                        if (isDebug && data.message) {
                            console.warn('[CwmScripture] Scripture fetch error:', data.message);
                        }
                    ...
                    if (attempt >= MAX_AUTO_RETRIES) {
                        if (isDebug) {
                            console.error('[CwmScripture] Scripture fetch failed:', error.message || error);
                        }
