...
class BibleProviderFactory
{
     ...
     * @param   Registry  $params       Params with provider_* settings
     *
    ...
    public static function getProviderForTranslation(string $version, Registry $params): BibleProviderInterface
    {
        ...
        if ($apiBibleEnabled && !empty($apiBibleKey)) {
            try {
                ...
                Log::add('DB error checking api_bible: ' . $e->getMessage(), Log::ERROR, 'cwmscripture.bible');
            }
        }
        ...
        if ($getbibleEnabled) {
            try {
                ...
                Log::add('DB error checking getbible: ' . $e->getMessage(), Log::ERROR, 'cwmscripture.bible');
            }
        }

        // Fallback
        Log::add('No provider matched "' . $version . '", falling back to ' . ($getbibleEnabled ? 'getbible' : 'local'), Log::WARNING, 'cwmscripture.bible');
